...
const Leaderboard = () => {
  ...
  const handleRecalculate = async () => {
    ...
    try {
      const { data } = await API.post(`/results/recalculate/${activeTab}`, {
        eventId,
      });
      dispatch(updateResults({ categoryId: activeTab, results: data.results }));
    } catch { /* silent */ }
      ...
      <div className="bg-white border-b px-6 py-5">
        <div className="max-w-5xl mx-auto flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-0.5">Live Leaderboard</p>
          </div>
      ...
      {isAdmin && (
        <div className="max-w-5xl mx-auto px-6 pt-5 flex gap-2">
          {['leaderboard', 'progress'].map((view) => (
            ...
            >
              {view === 'leaderboard' ? '🏆 Leaderboard' : '📊 Scoring Progress'}
            </button>
          ))}
        </div>
      )}
        ...
        {activeView === 'leaderboard' ? (
          <>
              ...
              <div className="grid grid-cols-3 gap-4 mb-6">
                <div className="bg-white rounded-xl border border-gray-100 p-4 text-center shadow-sm">
                  <p className="text-3xl font-bold text-yellow-500">
                    {categoryResults[0]?.participant?.name?.split(' ')[0] || '—'}
                  </p>
                  <p className="text-xs text-gray-400 mt-1">Current Leader</p>
                </div>
                <div className="bg-white rounded-xl border border-gray-100 p-4 text-center shadow-sm">
                  <p className="text-3xl font-bold text-blue-600">
                    {categoryResults[0]?.totalScore?.toFixed(1) || '—'}
                  </p>
            ...
            {categoryResults.length === 0 ? (
              <div className="text-center text-gray-400 py-20">
                ...
                <p className="text-sm">Results appear as judges submit scores</p>
              </div>
            ) : (
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            <h3 className="font-semibold text-gray-700 mb-4">
              Scoring Progress — {categories.find(c => c._id === activeTab)?.name}
            </h3>
